...
<!-- Fig1/Fig2 は `fig*_final.png`（キャッシュバスター）。`npm run copy:figures` で `public/figures/` に同期。Fig3/Fig4 は PNG またはページ内 React 図。 -->
...
![Figure 1: Processing pipeline comparison between LLM (token-based) and LCM (narrative-state-based).](figures/fig1_final.png)
...
![Figure 2: Narrative compilation pipeline from raw text to crystallized state hash.](figures/fig2_final.png)
