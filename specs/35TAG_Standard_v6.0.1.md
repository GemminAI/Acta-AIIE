# AIIE Protocol Specification: The 35TAG Standards (v6.0.1)
...
|**04**|`object_entity`|`array`|**Interference Image** $I(x) = \vert \Psi_{total} \vert^2$. The target entities (states, corporations) that crystallize as an intensity peak from the superposition of narrative wavefunctions. The target does not pre-exist as a fixed eigenket; it emerges through the interaction of observer namespace (T01), initial basis (T02), operator action (T03), and surrounding contextual fields. High $H_0$ (T22) diffuses the image; low $H_0$ crystallizes it. _(QFOM: RFC-0006 §4)_|
...
**Migration Protocol "Entropy-Shift" (RFC-0005).**
...
- DB migration: `narrative_states.t22_audit_aura` (string) → `t22_informational_entropy` (float) is deferred to PHASE D implementation.
- API: Laravel `EventController.store()` accepts `schema_version` from payload; BFF passes `informational_entropy` when present, falls back to legacy aura label.
...
_© 2026 Gemmina Intelligence LLC. — Protocol Standard v6.0.1_ _Acta AIIE Standardization Committee_
